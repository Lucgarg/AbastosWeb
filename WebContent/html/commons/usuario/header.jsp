...
<%Carrito carrito = (Carrito)SessionManager.get(request,AttributesNames.CARRITO);%>
       ...
       <%@include file="/html/commons/usuario/user-view.jsp" %>
           	...
           	<button class="cerrarLabel"></button>
           		...
           		<a href=<%=UrlBuilder.getUrlForController(request, ControllerPath.PRECREATE, ActionNames.REGISTRO, true, ParameterNames.TIP_USUARIO, ActionNames.EMPRESA)%>><button type="button" class="Buscar">Empresa</button></a>
           		<a href=<%=UrlBuilder.getUrlForController(request, ControllerPath.PRECREATE, ActionNames.REGISTRO, true, ParameterNames.TIP_USUARIO, ActionNames.PARTICULAR)%>><button type="button" class="Buscar">Particular</button></a>
           ...
           <div class="registro">
			...
			<form action="<%=UrlBuilder.getUrl(request, ControllerPath.USUARIO)%>" method="post">
				...
				id="particularLog" name="<%=ParameterNames.TIP_USUARIO%>" value="<%=ActionNames.PARTICULAR%>" checked> <label for="empresaLog">Empresa</label>
			<input type="radio" id="empresaLog" value="<%=ActionNames.EMPRESA%>" name="<%=ParameterNames.TIP_USUARIO%>"
			<%if(nameEmpresa != null){%>checked<%}%>><br>
			<label for="usuario">Usuario o email</label><br>
				 <input
					type="text" name="<%=ParameterNames.NOMBRE_USUARIO%>" <%if(name != null){%>value="<%=name%>"<%}else if(nameEmpresa != null){%>value="<%=nameEmpresa%>"<%}%>>
					<br> <label for="password" >Contraseña</label><br>
				
				<input type="password" name=<%=ParameterNames.PASSWORD%>><br> 
				<label>Recordar usuario</label>
				<input type="radio" name="<%=ParameterNames.MANTENER_RECORDAR%>" value="<%=ParameterNames.RECORDAR_USUARIO%>"><br>
					<label>Mantener sesión</label>
				<input type="radio" name="<%=ParameterNames.MANTENER_RECORDAR%>" value="<%=ParameterNames.MANTENER_SESION%>"> <br>
